...
console = Console()
def isextip(ip): #External IP check
    try:
...
def issigned(filepath):
    try:
        ...
        return False
def calcentropy(data): #Shannon entropy calc (not perfect)
    if not data:
        return 0.0
    return -sum((data.count(x) / len(data)) * math.log2(data.count(x) / len(data)) for x in range(256) if data.count(x))
def largestsectentropy(fp): #pefile target largest exec sect
    try:
        pe = pefile.PE(fp)
        ...
        return 0
def isddns(domain): #checks if host uses known DDNS
...
def longconn(conn): #>1hr con time check
    try:
        return (time.time() - psutil.Process(conn.pid).create_time()) > 3600
    except (psutil.NoSuchProcess, psutil.AccessDenied):
        return False
def nonstdport(port, svc): #Labels for standard port/svc
    stdports = {
...
def issusploc(path): #Highrisk filepaths
    locs = [r'\\temp\\', r'\\appdata\\', r'\\startup\\', r'\\programdata\\', r'\\users\\public\\']
    return any(loc in path.lower() for loc in locs)
def hasargs(cmd): #Sus ARGs
    args = ['-hidden', '-nowindow', '-silent', '-background','/hidden', '/nowindow', '/silent', '/background', '--hidden', '--nowindow', '--silent', '--background']
    return any(arg.lower() in cmd for arg in args)
def chkconn(conn): #main check
    ...
    try:
        ...
        if ent > 7.0: #can be 6.5 too, use 7 for lower fp
